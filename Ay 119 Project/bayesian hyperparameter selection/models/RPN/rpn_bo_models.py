...
from bofn.models.RPN.rpn_bo_architectures import MLP
...
class EnsembleRegression:
    ...
    def posterior_no_vmap(self, params, inputs):
        params, params_prior = params
        samples = []

        # Loop over each input while keeping params and params_prior constant
        for i in range(len(inputs)):
            # Call self.net_forward for each input with the same params and params_prior
            sample = self.net_forward(params, params_prior, inputs[i])
            samples.append(sample)

        # Stack the results back into a single array
